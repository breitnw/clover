...
-- by the user of the 'MonadBackend' instance.
class (Monad m, MonadError e m, Show e) => MonadBackend m e | m -> e where
  ...
  currentSongID :: m (Maybe (TrackID m))
  ...
  --
  -- Fails (via the MonadError instance) if the ID refers to an invalid song.
  getArtwork :: TrackID m -> m (Maybe STB.Image)
